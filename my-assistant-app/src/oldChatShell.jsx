 ...
 * @property {any[]} messages - Array of messages
 * @property {string} [status] - Optional status string
 * @property {string} [lastError] - Optional last error message
 */
...
// Create a message converter to transform agent messages to assistant-ui format
const messageConverter = React.useMemo(() => {
  return createMessageConverter((message) => {
    try {
      // Skip tool messages entirely - they're only for the backend
      if (message.role === 'tool') {
        return null; // This will filter out tool messages
      }
      ...
      if (message.parts && Array.isArray(message.parts)) {
        ...
        content = message.parts
          .map(part => {
            ...
            if (part.type === 'tool-call') {
              const toolCallId = part.toolCallId || part.id || `tool_${Date.now()}`;
              const toolName = part.toolName || part.name || 'unknown_tool';
              const args = part.args || part.arguments || {};
              
              // Only include tool-call if we have a valid tool name
              ...
              // If no valid tool name, convert to text
              return {
                type: 'text',
        ...
        // Convert string content to content format
      ...
      return {
        role: message.role || "assistant",
        content: [{ type: "text", text: "Error processing message" }]
      };
    }
  });
}, []);
...
// Define the state converter function with debugging
const converter = React.useCallback((
  state, 
  connectionMetadata
) => {
  // Extract pending messages from commands for optimistic updates
  const optimisticMessages = connectionMetadata.pendingCommands
    .filter((c) => c.type === "add-message")
    .map((c) => c.message);

  // Combine existing messages with optimistic updates
  const allMessages = [...(state.messages || []), ...optimisticMessages];
  
  // Filter out tool messages - they're only for the backend
  const filteredMessages = allMessages.filter(msg => msg.role !== 'tool');
  
  try {
    // Convert messages to assistant-ui format
    const convertedMessages = messageConverter.toThreadMessages(filteredMessages);
  ...
  // Create runtime for assistant-ui with Assistant Transport
  const runtime = useAssistantTransportRuntime({
    ...
    converter,
    headers: async () => ({
      "X-WP-Nonce": typeof fluentMcpAgent !== "undefined" ? fluentMcpAgent.nonce : undefined
    }),
    body: {
  ...
  // The rest of the JSX remains the same as in the original code
  return (
    <TooltipProvider>
